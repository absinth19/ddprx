                        ...
                        if should_include_channel(channelName, event_name, sport_key):
                            ...
                            if stream_url_dynamic:
                                ...
                                with open(M3U8_OUTPUT_FILE, 'a', encoding='utf-8') as file:
                                    ...
                                    file.write(f'#EXTINF:-1 tvg-id="{event_name} - {event_details.split(":", 1)[1].strip() if ":" in event_details else event_details}" tvg-name="{tvg_name}" tvg-logo="{event_logo}" group-title="{clean_sport_key}", {tvg_name} - {channel_name_str}\n')
